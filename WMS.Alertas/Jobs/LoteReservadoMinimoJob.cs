...
/// <summary>
/// Procesa las reservas que requieren actualización y envía un informe
/// consolidado a los destinatarios configurados para la alerta.
/// </summary>
public class LoteReservadoMinimoJob
{
    ...
    public async Task Ejecutar()
    {
        ...
        try
        {
            ...
            await EjecutarConsolidado(logId, lotes);
        }
        catch (Exception ex)
        {
            if (logId == 0)
    ...
    private async Task EjecutarConsolidado(
        int logId,
        List<LoteReservadoMinimo> lotesMinimo)
    {
        var diagnostico = await _controlVencimientosService.ObtenerLotesReservados();
        ...
        if (_configuracionEjecucion.EsProduccion && total == 0 && errores.Count == 0)
        {
        ...
        var destinatarios = await _correoDestinoService.ObtenerCorreos(
            "LoteReservados");
        destinatarios.AddRange(lotesMinimo.Select(x => x.SolicitanteCorreo));
        destinatarios.AddRange(
            diagnostico.SolicitudesVigenciaVencida.Select(x => x.SolicitanteCorreo));
        destinatarios.AddRange(
            diagnostico.SolicitudesLote.Select(x => x.SolicitanteCorreo));
        ...
        if (destinatarios.Count == 0)
            throw new InvalidOperationException(
                "No hay destinatarios configurados para LoteReservados.");
