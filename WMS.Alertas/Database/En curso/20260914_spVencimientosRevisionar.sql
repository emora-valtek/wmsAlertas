...
BEGIN
    ...
    BEGIN TRY
        ...
        SELECT TOP (@CantidadMaxima)
            ...
            ,DATEADD(DAY, -P.PRO_MinimoVencimiento, L.LOT_FechaVencimiento) AS FechaEnvioRevision
        INTO #Cambios
        FROM dbo.XTN_Existencia X WITH (UPDLOCK, READPAST, ROWLOCK)
        INNER JOIN dbo.PRO_Producto P ON P.PRO_Id = X.PRO_Id AND P.PRO_Activo = 1
        INNER JOIN dbo.LOT_Lote L ON L.LOT_Id = X.LOT_Id AND L.LOT_Activo = 1
        LEFT JOIN dbo.NIV_Nivel N ON N.NIV_Id = X.NIV_Id
        ...
        BEGIN
            ...
            SET XTN_Estado = 11,
                MREV_Id = 9000,
                XTN_ModificadoPor = @UsuarioId,
                XTN_FechaModificacion = GETDATE()
            FROM dbo.XTN_Existencia X
            INNER JOIN #Cambios C ON C.ExistenciaId = X.XTN_Id
            WHERE X.XTN_Estado = C.EstadoAnterior;

            EXEC sys.sp_set_session_context @key = N'OmitirTriggerStock', @value = NULL;

            INSERT dbo.StockMovimiento
            (
                Fecha, UsuarioId, ExistenciaId, ProductoId, LoteId,
                NivelMovimiento, TipoMovimiento, Proceso, Cantidad,
                DocumentoOrigenTipo, DocumentoOrigenId
            )
            SELECT GETDATE(), @UsuarioId, ExistenciaId, ProductoId, LoteId,
                'EXISTENCIA',
                CASE WHEN EstadoAnterior = 2 THEN 'SALIDA' ELSE 'PROCESO_INTERNO' END,
                'ENVIAR_REVISION', 1, 'REVISION_AUTOMATICA', ExistenciaId
            FROM #Cambios;
            ...
            SELECT GETDATE(), @UsuarioId, NULL, ProductoId, LoteId,
                'PRODUCTO',
                CASE WHEN EstadoAnterior = 2 THEN 'SALIDA' ELSE 'PROCESO_INTERNO' END,
                'ENVIAR_REVISION', COUNT(*), 'REVISION_AUTOMATICA', NULL
            FROM #Cambios
            GROUP BY ProductoId, LoteId, EstadoAnterior;

            UPDATE PLC
            SET PROLOTCANT_Recepcionados = CASE WHEN ISNULL(PROLOTCANT_Recepcionados, 0) >= A.R THEN ISNULL(PROLOTCANT_Recepcionados, 0) - A.R ELSE 0 END,
                ...
                PROLOTCANT_ModificadoPor = @UsuarioId,
                PROLOTCANT_FechaModificacion = GETDATE()
            FROM dbo.PROLOTCANT_ProductoLoteCantidad PLC
            INNER JOIN
            (
                SELECT ProductoId, LoteId,
                    SUM(CASE WHEN EstadoAnterior = 0 THEN 1 ELSE 0 END) AS R,
                    SUM(CASE WHEN EstadoAnterior = 1 THEN 1 ELSE 0 END) AS P,
                    SUM(CASE WHEN EstadoAnterior = 2 THEN 1 ELSE 0 END) AS D
                FROM #Cambios
                GROUP BY ProductoId, LoteId
            ) A ON A.ProductoId = PLC.PRO_Id AND A.LoteId = PLC.LOT_Id
            WHERE PLC.PROLOTCANT_Activo = 1;
        ...
        SELECT @Procesadas AS Procesadas, @Pendientes AS Pendientes;
    END TRY
    BEGIN CATCH
        EXEC sys.sp_set_session_context @key = N'OmitirTriggerStock', @value = NULL;
